...
class runningRecord:
    ...
    def __str__(self):
        return 'startTime: ' + self.startTime + ' endTime: ' + self.endTime + ' host: ' + self.host + ' duration: ' + self.duration
        ...
        for job in self.root.findall('Job'):
            ...
            for child in job:
                if child.tag == 'RunningRecord':
                    job_dict.append(runningRecord(child.attrib['start'], child.attrib['end'], child.attrib['host'], child.attrib['duration']))
