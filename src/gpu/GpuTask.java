...
public class GpuTask {
	...
	private List<GpuTask> children;

	private List<GpuTask> parent;
	...
	public GpuTask(final int taskId, final long blockLength, final int numberOfBlocks, final long inputSize,
		...
		this.taskInputSize = inputSize;
		this.taskOutputSize = outputSize;
	 ...
	 * @param p 父任务
	 */
	public void addParent(GpuTask p) {
		this.parent.add(p);
	}
	 ...
	 * @param c 子任务
	 */
	public void addChild(GpuTask c) {
		this.children.add(c);
	}
	 ...
	 * @return
	 */
	public List<GpuTask> getChildren() {
		return children;
	}
    ...
    public void reset() {
		this.status = CREATED;
		if(index != -1)
			resList.get(index).finishedSoFar = 0;
		setExecStartTime(-1);
		setFinishTime(-1);
    }
...
//		return getBlockLength() * getNumberOfBlocks() * getThreadsPerBlock();
	 ...
	 * 
	 * @param resourceID the CloudResource ID
	 * @param costPerGPU the cost per second of running this task
	 ...
	 * 
	 * @param utilizationModelGpu the new utilization model of gpu
	 */
	 ...
	 * 
	 * @param utilizationModelGddram the new utilization model of gddram
	 */
	...
	public double getD2h() {
		return d2h;
	}

	public double getH2d() {
		return h2d;
	}

	public void setD2h(double d2h) {
		this.d2h = d2h;
	}

	public void setH2d(double h2d) {
		this.h2d = h2d;
	}
}
